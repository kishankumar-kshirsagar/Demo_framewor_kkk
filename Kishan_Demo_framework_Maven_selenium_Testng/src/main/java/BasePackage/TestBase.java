...
public class TestBase extends WebDriverTestBase {
	...
	public static void Verify_the_Table_Headers(List<String> expectedHeaders, List<WebElement> headers) {
		...
		int total_headers = headers.size(); //actual headers size
		System.out.println("The value of the actuals headers are - "+total_headers);
		softAssert.assertEquals(expectedHeader, total_headers, "Number of headers count mismatch");// Soft assert for header size
		for (int i = 0; i < total_headers; i++) {// Iterate through each header element
			...
			System.out.println("Actual header is "+ actualHeaderText+ " at index  "+i );
			String expectedHeaderText = expectedHeaders.get(i);
			System.out.println("Expected header is "+ expectedHeaderText+ " at index "+ i );
